...
export default function AddInven() {
  const user = getUser();
  ...
  let handleSubmit = async (e) => {
    ...
    console.log(  serial_ar)

    if (selectedVendor && selectedMaterial && selectedModel && selectedProject && selectedPO &&  serial_ar) {
      try {
        let res = await fetch(`https://sarmicrosystems.in/react_inventory/API/addInventory.php?vendor=` + selectedVendor + `&material=` + selectedMaterial + `&model=` + selectedModel + `&po=` + selectedPO + `&project=` + selectedProject + `&invoice=` + invoice + `&quantity=` + number + `&serial_num=` + serialNumbers + `&created_by=` + user)
                                ...
                                {fields.map((str, index) => (
                                  <>
                                    ...
                                    <input
                                      className="form-control"
                                      type="text"
                                      value={check ? serialNumbers[index] || '' : ''}
